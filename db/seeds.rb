...
avatar = Movie.create!(title: "Avatar 2", genre_one: 'Sci-Fi', genre_two: 'Adventure', year: 2022, description: "A sequel to Avatar (2009).", writer_id: jamesCameron.id, director_id: jamesCameron.id, actor_id: kateWinslet.id)
avatar.photo.attach(io: open("https://eyemdbee-dev.s3.us-east-2.amazonaws.com/movies/avatar.jpg"), filename:'avatar.jpg')

bobsBurgers = Movie.create!(title:"Bob's Burgers: The Movie", genre_one: 'Animation', genre_two: 'Comedy', year: 2022, description:"Plot unknown. Feature film based on the animated TV series.", writer_id: lorenBouchard.id, director_id: lorenBouchard.id, actor_id: hJonBenjamin.id)
...
houseOfGucci = Movie.create!(title:"House of Gucci", genre_one: 'Crime', genre_two: 'Drama', year: 2021, film_rating: "  •  "+"R", writer_id: robertoBentivegna.id, director_id: ridleyScott.id, actor_id: adamDriver.id, description:"House of Gucci is inspired by the family empire behind the Italian fashion house of Gucci. Spanning three decades of love, betrayal, decadence, revenge, and ultimately murder, the film reveals what a name means, what it's worth, and how far a family will go for control.")
...
jurassicWorld = Movie.create!(title:"Jurassic World: Dominion", genre_one: 'Sci-Fi', genre_two: 'Action', year: 2022, writer_id: colinTrevorrow.id, director_id: colinTrevorrow.id, actor_id: chrisPratt.id, description:"Plot kept under wraps.")
...
kingRichard = Movie.create!(title:"King Richard", genre_one: 'Biography', genre_two: 'Drama', year: 2021, film_rating: "  •  "+"PG-13"+"  •  ", duration: "2h 18min", writer_id: zachBaylin.id, director_id: reinaldoMarcusGreen.id, actor_id: willSmith.id, description:"A look at how tennis superstars Venus and Serena Williams became who they are after the coaching from their father Richard Williams.")
...
spencer = Movie.create!(title:"Spencer", genre_one: 'Drama', genre_two: 'Romance', year: 2021, film_rating: "  •  "+"R"+"  •  ", duration: "1h 51min", writer_id: stevenKnight.id, director_id: pabloLarraín.id, actor_id: kristenStewart.id, description:"During her Christmas holidays with the royal family at the Sandringham estate in Norfolk, England, Diana decides to leave her marriage to Prince Charles.")
spencer.photo.attach(io: open("https://eyemdbee-dev.s3.us-east-2.amazonaws.com/movies/spencer.jpg"), filename:'spencer.jpg')

spiderman = Movie.create!(title:"Spider-Man: No Way Home", genre_one: 'Action', genre_two: 'Adventure', year: 2021, writer_id: chrisMckenna.id, director_id: jonWatts.id, actor_id: zendaya.id, description:"For the first time in the cinematic history of Spider-Man, our friendly neighborhood hero is unmasked and no longer able to separate his normal life from the high-stakes of being a Super Hero. When he asks for help from Doctor Strange, the stakes become even more dangerous, forcing him to discover what it truly means to be Spider-Man.")
spiderman.photo.attach(io: open("https://eyemdbee-dev.s3.us-east-2.amazonaws.com/movies/spiderman.jpg"), filename:'spiderman.jpg')

theFrenchDispatch = Movie.create!(title:"The French Dispatch", genre_one: 'Comedy', genre_two: 'Drama', year: 2021, film_rating: "  •  "+"R"+"  •  ", duration: "1h 48min", writer_id: wesAnderson.id, director_id: wesAnderson.id, actor_id: léaSeydoux.id, description:"A love letter to journalists set in an outpost of an American newspaper in a fictional twentieth century French city that brings to life a collection of stories published in 'The French Dispatch Magazine'.")
...
theKingsMan = Movie.create!(title:"The King's Man", genre_one: 'Action', genre_two: 'Adventure', year: 2021, film_rating: "  •  "+"R", writer_id: matthewVaughn.id, director_id: matthewVaughn.id, actor_id: ralphFiennes.id, description:"In the early years of the 20th century, the Kingsman agency is formed to stand against a cabal plotting a war to wipe out millions.")
...
the355 = Movie.create!(title:"The 355", genre_one: 'Action', genre_two: 'Thriller', year: 2022, film_rating: "  •  "+"PG-13", writer_id: simonKinberg.id, director_id: simonKinberg.id, actor_id: lupitaNyongo.id, description:"When a top-secret weapon falls into mercenary hands, wild card CIA agent Mason 'Mace' Brown will need to join forces with rival badass German agent Marie, former MI6 ally and cutting-edge computer specialist Khadijah, and skilled Colombian psychologist Graciela on a lethal, breakneck mission to retrieve it, while also staying one-step ahead of a mysterious woman, Lin Mi Sheng, who is tracking their every move. As the action rockets around the globe from the cafes of Paris to the markets of Morocco to the wealth and glamour of Shanghai, the quartet of women will forge a tenuous loyalty that could protect the world-or get them killed.")
the355.photo.attach(io: open("https://eyemdbee-dev.s3.us-east-2.amazonaws.com/movies/the355.jpg"), filename:'the355.jpg')

topGun = Movie.create!(title:"Top Gun: Maverick", genre_one: 'Action', genre_two: 'Drama', year: 2022, film_rating: "  •  "+"PG-13", writer_id: christopherMcQuarrie.id, director_id: josephKosinski.id, actor_id: tomCruise.id, description:"After more than thirty years of service as one of the Navy's top aviators, Pete Mitchell is where he belongs, pushing the envelope as a courageous test pilot and dodging the advancement in rank that would ground him.")
topGun.photo.attach(io: open("https://eyemdbee-dev.s3.us-east-2.amazonaws.com/movies/top-gun.jpg"), filename:'top-gun.jpg')

westSideStory = Movie.create!(title:"West Side Story", genre_one: 'Musical', genre_two: 'Drama', year: 2021, film_rating: "  •  "+"PG-13", writer_id: tonyKushner.id, director_id: stevenSpielberg.id, actor_id: anselElgort.id, description:"An adaptation of the 1957 musical, West Side Story explores forbidden love and the rivalry between the Jets and the Sharks, two teenage street gangs of different ethnic backgrounds.")
